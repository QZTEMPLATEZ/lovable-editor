...
const StepIndicator = ({ currentStep, steps }: StepIndicatorProps) => {
  ...
  return (
    <div className="relative mb-8 bg-editor-bg p-6 rounded-lg border border-editor-border">
      ...
      />
      <motion.div 
        className="absolute top-[2.5rem] left-0 h-[2px] bg-editor-glow-pink"
        initial={{ width: '0%' }}
          ...
          >
            ...
            >
              <div 
                className={`w-8 h-8 flex items-center justify-center relative transition-all duration-300
                  ${index <= currentStep 
                    ? 'bg-editor-panel border-2 border-editor-glow-pink shadow-lg shadow-editor-glow-pink/50' 
              ...
              >
                {index < currentStep ? (
                  ...
                  >
                    <Check className="w-4 h-4 text-editor-glow-pink" />
                  </motion.div>
                ) : (
                  <span className={`text-xs font-medium ${index === currentStep ? 'text-editor-glow-pink' : 'text-gray-400'}`}>
                    {index + 1}
                  </span>
                )}
                ...
                {index === currentStep && (
                  <>
                    <div 
                      className="absolute inset-0 animate-ping opacity-20 border-2 border-editor-glow-pink"
                      style={{
                        clipPath: 'polygon(50% 0%, 100% 25%, 100% 75%, 50% 100%, 0% 75%, 0% 25%)',
                      }}
                    />
                    <div 
                      className="absolute -inset-1 animate-pulse opacity-10 border-2 border-editor-glow-pink"
                      style={{
                        clipPath: 'polygon(50% 0%, 100% 25%, 100% 75%, 50% 100%, 0% 75%, 0% 25%)',
                      }}
                    />
            ...
            >
              <h3 className={`text-sm font-medium mb-1 transition-colors duration-300
                ${index <= currentStep ? 'text-editor-glow-pink' : 'text-gray-400'}
