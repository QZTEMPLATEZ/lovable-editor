...
const EditorHeader = ({ editingMode, targetDuration, onDurationChange }: EditorHeaderProps) => {
  return (
    <div className="text-center space-y-8">
      <div className="max-w-6xl mx-auto bg-purple-950/5 backdrop-blur-sm rounded-3xl p-12 border border-purple-500/10 shadow-xl">
        <div className="flex flex-col items-center gap-10">
          ...
          >
            <TabsList className="grid grid-cols-2 lg:grid-cols-4 gap-4 w-full bg-purple-950/10 p-4 rounded-2xl">
              {VIDEO_DURATIONS.map((duration) => (
                <TabsTrigger
                  key={`${duration.min}-${duration.max}`}
                  value={`${duration.min}-${duration.max}`}
                  className="relative group py-8 px-6 data-[state=active]:bg-purple-500/90 data-[state=active]:shadow-lg data-[state=active]:backdrop-blur-sm data-[state=active]:text-white transition-all duration-300 rounded-xl hover:bg-purple-500/20"
                >
                  <div className="flex flex-col items-center gap-3">
                    {duration.min === 4 && (
                      <Badge variant="secondary" className="absolute -top-3 text-[10px] bg-gradient-to-r from-purple-400 to-pink-400 text-white font-medium px-2.5 py-0.5 shadow-lg rounded-full border border-white/10 animate-pulse">
                        PRO
                      </Badge>
                    )}
                    {duration.min === 8 && (
                      <Badge variant="secondary" className="absolute -top-3 text-[10px] bg-gradient-to-r from-purple-600 to-pink-600 text-white font-medium px-2.5 py-0.5 shadow-lg rounded-full border border-white/10">
                        PRO MAX
                      </Badge>
                    )}
                    {(duration.min === 15 || duration.min === 30) && (
                      <Badge variant="secondary" className="absolute -top-3 text-[10px] bg-gradient-to-r from-yellow-400 to-orange-400 text-white font-medium px-2.5 py-0.5 shadow-lg rounded-full border border-white/10">
                        BUSINESS
                      </Badge>
                    )}
